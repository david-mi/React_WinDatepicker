...
import { getDates, checkIfDatesAreIdentical, checkIfDateIsOutsideRange, checkIfDateIsFromWeekend } from "../../lib/WinDate/Calendar/Month/helper";
...
describe("checkIfDateIsFromWeekend helper", () => {
  it("Should be false for Monday", () => {
    const date = new Date("2023/05/01")

    expect(checkIfDateIsFromWeekend(date)).toBe(false)
  })

  it("Should be false for Tuesday", () => {
    const date = new Date("2023/05/02")

    expect(checkIfDateIsFromWeekend(date)).toBe(false)
  })

  it("Should be false for Wednesday", () => {
    const date = new Date("2023/05/03")

    expect(checkIfDateIsFromWeekend(date)).toBe(false)
  })

  it("Should be false for Thursday", () => {
    const date = new Date("2023/05/04")

    expect(checkIfDateIsFromWeekend(date)).toBe(false)
  })

  it("Should be false for Friday", () => {
    const date = new Date("2023/05/05")

    expect(checkIfDateIsFromWeekend(date)).toBe(false)
  })

  it("Should be true for Saturday", () => {
    const date = new Date("2023/05/06")

    expect(checkIfDateIsFromWeekend(date)).toBe(true)
  })

  it("Should be true for Sunday", () => {
